...
class EGB{
	...
	public function get_fac_advisory($sy, $deptcode, $level, $seccode, $period){
		...
		*/
		...
		$sql .="FROM erb_stsn.nrol_currhdr   LEFT JOIN erb_stsn.nrol_currdtl  ON (nrol_currhdr.CurriculumId = nrol_currdtl.CurriculumId) LEFT JOIN erb_stsn.tb_mastersection ";
        $sql .="ON (tb_mastersection.Level = nrol_currdtl.CurrYear)  INNER JOIN erb_stsn.tb_subject ON (nrol_currdtl.CompCode = tb_subject.CompCode) AND (tb_mastersection.DeptCode = nrol_currhdr.ProgramId) ";
		$sql .="LEFT JOIN erb_stsn.nrol_posted   ON (nrol_currhdr.SY = nrol_posted.SY) AND (tb_mastersection.SectionCode = nrol_posted.SectionCode) AND (nrol_posted.CompCode = tb_subject.CompCode) ";
		$sql .="WHERE (tb_mastersection.SectionCode ='$seccode'   AND nrol_currhdr.SY ='$sy') ORDER BY nrol_currdtl.IndexOrder ASC ";
		$results=array();
	...
	public function isPosted($period, $sy, $seccode, $compcode){
		...
		$sql .= " WHERE     (Period = $period) AND (CompCode = '$compcode') AND (SectionCode = '$seccode') AND (SY = $sy) AND StatusCode > 0";
		if ($stmt = $this->db_connection->prepare($sql)) {
			$stmt->execute();
			$stmt->bind_result($results);
			$stmt->fetch();
	...
	public function get_cgs_scores($sy, $period, $seccode){
		...
		$query .="INNER JOIN erb_stsn.tb_subject         ON (tb_subject.CompCode = nrol_currdtl.CompCode)    LEFT JOIN erb_stsn.tb_temporary_grade         ON (tb_temporary_grade.SectionCode = nrol_advisory_class.SectionCode) AND (tb_temporary_grade.CompCode = tb_subject.CompCode) ";
	...
	public function get_final_scores($sy, $period, $seccode, $sno=''){
		...
		$query .="FROM    erb_stsn.nrol_advisory_class    INNER JOIN erb_stsn.tb_mastersection         ON (nrol_advisory_class.SectionCode = tb_mastersection.SectionCode) ";
		$query .="INNER JOIN erb_stsn.nrol_currhdr         ON (tb_mastersection.DeptCode = nrol_currhdr.ProgramId)    INNER JOIN erb_stsn.nrol_currdtl         ON (tb_mastersection.Level = nrol_currdtl.CurrYear) AND (nrol_currhdr.CurriculumId = nrol_currdtl.CurriculumId) ";
		$query .="INNER JOIN erb_stsn.tb_subject         ON (tb_subject.CompCode = nrol_currdtl.CompCode)    LEFT JOIN erb_stsn.tb_final_grade         ON (tb_final_grade.SectionCode = nrol_advisory_class.SectionCode) AND (tb_final_grade.CompCode = tb_subject.CompCode) ";
		$query .="WHERE (nrol_advisory_class.SY =$sy    AND nrol_advisory_class.Period =$period    AND nrol_advisory_class.SectionCode ='$seccode') ";
		if($sno!=''){
