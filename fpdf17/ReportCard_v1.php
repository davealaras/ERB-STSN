...
class ReportCard extends FPDF{
	...
	private function draw_hs_gs($subjects,$attendance,$conduct_tmplt,$student,$adviser,$section_alias,$dept_alias){
		//Main Boxes
		if($this->showLines){
			$this->Rect(0.3,2.00,7.07, 2.41); // Grades
		...
		//Top
		$this->SetFont('Arial','',9); //Font
		if($this->showLines){$this->right_text(1.02,1.10,'Name: ');}
		$this->Text(1.02,1.10,$student['fullname']);
		if($this->showLines){$this->right_text(1.52,1.27,'Year & Section: ');}
		$this->Text(1.52,1.27,$section_alias);
		
		if($this->showLines){$this->right_text(1.47,1.44,'Class Adviser: ');}
