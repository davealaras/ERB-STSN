...
var AVERAGE_ROUND =0;
	...
	var adjustmentshtml  = $('#div-adjustments').html();
	...
	function load_routine(classcode,sy, period,level, dept){
			...
			$('#div-adjustments').html('').html(adjustmentshtml);
		...
		$.ajax({
				...
				success: function(data) {
					...
					$.each(components,function(i,result){
						...
						if(isposted){
							$('<td><div class="medium right txt_right"colnum="'+(i+1)+'" classcode="'+ccode+'" count="'+count+'" ><span class="summarybox"></span><div class="adjustsign">+</div><span isposted="'+isposted+'" class="micro adjustbox" >0.00</span></div></td>').insertBefore('#adjustments .end');
						}else{
							$('<td><div class="medium right txt_right"colnum="'+(i+1)+'" classcode="'+ccode+'" count="'+count+'" ><span class="summarybox"></span><div class="adjustsign">+</div><input isposted="'+isposted+'" class="micro adjustbox" type="text"/></div></td>').insertBefore('#adjustments .end');	
						}
					});
					$('#summary .students .end').html('<div class="small totalbox right">'+gtotal+' </div>');
					$('#adjustments .students .end').html('<div class="small totalbox right"></div>');
					$('#summary .students .end').parent().append('<td><div class="small finalbox right">'+roundNumber(gtotal,0)+' </div></td>');
					$('#adjustments .students .end').parent().append('<td><div class="small finalbox right"></div></td>');
					...
					$.each(json_data.measurables,function(i,result){
						...
						if(!isposted){
							$('<td style="text-align:center;"><input value=" " isposted="'+isposted+'"type="text" class="micro smartbox"  saveas="rawscores" hdr="'+hdr+'" key="'+key+'" ccode="'+ccode+'"colnum="'+(i+1)+'"maxitems="'+itm+'" base="'+bse+'"  /></td>').insertBefore('#rawscores .end');
							
						}else{
							$('<td style="text-align:center;"><span isposted="'+isposted+'" type="text" class="micro smartbox" saveas="rawscores" hdr="'+hdr+'" key="'+key+'" ccode="'+ccode+'"colnum="'+(i+1)+'"maxitems="'+itm+'" base="'+bse+'"></span></td>').insertBefore('#rawscores .end');
						}
						$('<td><div class="micro dumbbox right failgrade" style="text-align:right" colnum="'+(i+1)+'"maxitems="'+itm+'"  ccode="'+ccode+'" base="'+bse+'" count="'+count+'" headername="'+hdr+'" key="'+key+'" >'+bse+'</div></td>').insertBefore('#equivalent .end');	
					});
		...
		$.ajax({
				...
				$.each(students,function(i,res){
						...
						if(curr_g_flg!=prev_g_flg){
							...
							row = '<tr class="students" rownum="'+i+'" id="rw-'+i+'" status="'+status+'" >';
	...
	function populate_smartbox(){
		...
		$.ajax({
				...
				success: function(data) {
					...
					var a_summaryboxes = $('#adjustments').find('.summarybox');
					var a_adjustboxes = $('#adjustments').find('.adjustbox');
					...
					//Tag id to summary dumbboxes
					$.each(s_dumbboxes, function(i,res){
						var header = $(res).attr('classcode');
						var row = $(res).parent().parent();
						var studentnumber = $(row).find('.studno').text();
						$(res).attr('id', 'sm-'+studentnumber+'-'+header);
					});
					
					//Tag id to adjustment summaryboxes
					$.each(a_summaryboxes, function(i,res){
						var header = $(res).parent().attr('classcode');
						var row = $(res).parent().parent().parent();
						var studentnumber = $(row).find('.studno').text();
						$(res).attr('id', 'ad-'+studentnumber+'-'+header);
					}); 
					
					//Tag id to adjustment summaryboxes
					$.each(a_adjustboxes, function(i,res){
						var col =  $(res).parent().attr('colnum');
						var row = $(res).parent().parent().parent().attr('rownum');
						$(res).attr('id', 'aj-'+row+'-'+col);
					}); 
					
					if(json_data.adjustments!=null){
						$.each(json_data.adjustments, function(i,record){
							var sno = record.sno;
							var classcode = record.key;
							var adjustment = record.adjustment;
							var sum_bx = $('#sm-'+sno+'-'+classcode);
							var adj_bx = $('#ad-'+sno+'-'+classcode);
							var trgt_bx =$(adj_bx).parent().find('.adjustbox');
							$(sum_bx).attr('adjmnts',adjustment);
							if($(trgt_bx).attr('isposted')=='true'){
								$(trgt_bx).text(adjustment);
								parse_adjust($(trgt_bx),1);
							}else{
								$(trgt_bx).val(adjustment).change();
							}
						});
					}
					...
					if(json_data.summary!=null){
						$.each(json_data.summary, function(i,record){
							var sno = record.sno;
							var hdr = record.hdr;
							var value = parseFloat(record.summary);
							var res = $('#sm-'+sno+'-'+hdr);
							var adj_bx = $('#ad-'+sno+'-'+hdr);
							var adjmnts = $(res).attr('adjmnts')?parseFloat($(res).attr('adjmnts')):0;
							var rownum = $(res).parent().parent().attr('rownum');
							if($(res)){
								$(res).text(roundNumber(value + adjmnts,SUMMARY_ROUND));
								$(adj_bx).text(roundNumber(value,SUMMARY_ROUND));
								var total=0;
								var t=0;
								$.each(s_dumbboxes,function(i, res){
									...
									if(rn==rownum){
										...
										v = isNaN(v)?0:v;
										total+=v;
										t = roundNumber(total,2);
										$(r).find('.totalbox').text(t);
										$('#adjustments #rw-'+rownum).find('.totalbox').text(t);
										$(r).find('.finalbox').attr('finalgrade',t);
										$(r).find('.finalbox').text(roundNumber(t,0));
										$('#adjustments #rw-'+rownum).find('.finalbox').text(roundNumber(t,0));
									}
								});
							}						
						});
					...
					$('#adjustments').ztable({tableWidth: ztb_w, tableHeight: ztb_h, columnCount:2});
	...
	//Adjust Box Keyup
	$('.adjustbox').livequery('keyup',function(e){
		var bx_inf =  $(this).attr('id').split('-');
		var row = bx_inf[1];
		var col = bx_inf[2];
		
		//START Navigation Behaviour
		if(e.which==13){
			var r= parseInt(row);
			var c = parseInt(col);
			var target = $('.ztable-data-container #adjustments #aj-'+(r+1)+'-'+c);
			console.log(target);
			if(target.length!=0){
				$(target).focus();
			}else{
				r=0;
				console.log('.ztable-data-container #adjustments #aj-'+r+'-'+(c+1));
				$('.ztable-data-container #adjustments #aj-'+r+'-'+(c+1)).focus();
			}
			...
			var sy = $('#sy').find('option:selected').val();
			var period = $('#period').find('option:selected').val();
			var bx_dtl = $(this).parent().find('.summarybox').attr('id').split('-');
			var sno = bx_dtl[1];
			var hdr = bx_dtl[2];
			var summary = parseFloat($(this).prev().prev().text());
			var adjustment = parseFloat($(this).prev().text()+$(this).val());
			var target = $('.ztable-data-container #summary #sm-'+sno+'-'+hdr);
			$(target).attr('adjmnts',adjustment);
			$(target).text(roundNumber(summary+adjustment,SUMMARY_ROUND));
			
			$.ajax({
					type: 'GET',
					url: 'ajax.php',
					data: {'classcode' :classcode, 'sy':sy, 'period': period, 'sno': sno, 'hdr': hdr, 'adjustment': adjustment, 'func':'saveadjustment'},
					success: function(data) {
					}
			});
			$(this).blur();
			...
			//RECOMPUTE TOTAL
			var target_row =  $('.ztable-data-container #summary #rw-'+row);
			var t_dbx = $(target_row).find('.totalbox');
			var f_dbx = $(target_row).find('.finalbox');
			var s_dbxs = $(target_row).find('.dumbbox');
			var total=0;
			$.each(s_dbxs, function(i,r){
				var value = parseFloat($(r).text());
				total+=value;
			});
			
			//REPLENISH Grade Entry
			total =  roundNumber(total,2);
			$(t_dbx).text(total);
			$(f_dbx).text(roundNumber(total,0));
			
			//REPLENISH Adjustments
			target_row =  $('.ztable-data-container #adjustments #rw-'+row);
			t_dbx = $(target_row).find('.totalbox');
			f_dbx = $(target_row).find('.finalbox');
			total =  roundNumber(total,2);
			$(t_dbx).text(total);
			$(f_dbx).text(roundNumber(total,0));
			
		}
		// END Navigation Behaviour
	});
	
	//Adjust Box Focus
	$('.adjustbox').livequery('focus',function(e){
		var bx_inf =  $(this).attr('id').split('-');
		var row = bx_inf[1];
		//UI Behaviour
		$('.ztable-v-column-container #adjustments #rw-'+row).addClass('highlight');
		$('.ztable-data-container #adjustments #rw-'+row).addClass('highlight');
		$(this).select();
	});
	
	//Adjust Box Blur
	$('.adjustbox').livequery('blur',function(e){
	
		//UI Behaviour
		var bx_inf =  $(this).attr('id').split('-');
		var row = bx_inf[1];
		$('.ztable-v-column-container #adjustments #rw-'+row).removeClass('highlight');
		$('.ztable-data-container #adjustments #rw-'+row).removeClass('highlight');
			
	});
	//Adjust Box Change
	$('.adjustbox').livequery('change',function(e){
		parse_adjust(this,0);
	});
	function parse_adjust(obj,flag){
		//PARSING
		var value = flag==0?$(obj).val():$(obj).text();
		var target =$(obj).parent().find('.adjustsign');
		$(target).text(value<0?'-':'+');
		var disp = roundNumber(Math.abs(value),SUMMARY_ROUND);
		flag==0?$(obj).val(disp):$(obj).text(disp);
	}
	// Make bad grades red
	...
	function useformula(rawscore, perfectscore,classcode,base, res,roundvalue){
				...
				if(rawscore==-3){
					...
					$(res).text('IGN');
					$(res).removeClass('failgrade');
				}else if(classcode !='LT' && classcode !='SP'&& classcode !='QZ' && classcode !='QE'){
					// Regular formula
					...
					if(rawscore>=factor){
						value = roundNumber((rawscore-factor) * (24/factor)+75,0);
						value = roundNumber(value,roundvalue);
						$(res).text(value);
						failGrade(res);
						
					}else  if(rawscore< factor && rawscore>=0){
						value = roundNumber(rawscore * (4/factor)+70,0);
						value = roundNumber(value,roundvalue);
						$(res).text(value);
						failGrade(res);
					}
	...
	function smart_compute(itemscore,sum,oall, rownum, colnum, ccode){
		...
		var vv=$(cell).text();
		var ave = roundNumber(parseFloat($(cell).text()),AVERAGE_ROUND);
		var adjust = parseFloat($(cell).attr('adjmnts'));
		var unadjusted_summary = roundNumber(ave*(p/100),SUMMARY_ROUND);
		var adjusted_summary = roundNumber((ave*(p/100))+adjust,SUMMARY_ROUND);
		$('.ztable-data-container #adjustments #ad-'+sno+'-'+hdr).text(unadjusted_summary);
		$(cell).text(adjusted_summary);
		...
		$.each(S,function(i, res){
			...
			if(rn==rownum){
					...
					value = isNaN(value)?0:value;
					total+=value;
					t = roundNumber(total,SUMMARY_ROUND);
					var tb = $(r).find('.totalbox');
					var fb = $(r).find('.finalbox');
					tb.text(t); // totalbox
					fb.text(roundNumber(t,OVERALL_ROUND)); // finalbox
					//failGrade(tb); // COLORIZE WHEN FAIL
			}
		});
		var selected_period = $('#period').find('option:selected').val();
		//Save total
	...
	$('.export_btn').click(function(e){
		...
		console.log('.ztable-data-container #'+datasource+' tr');
		...
		$.each(rows, function(i,r){
			...
			$.each(cols, function(j,s){
				var cell=$(s).find('input').val();
				cell=cell==undefined?$(s).text():cell;
				arr_row.push(cell);
			});
