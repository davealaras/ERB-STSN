$(document).ready(function() {
		...
		if(source=='tbGenComp'){
			...
			if(v =="###"){
				...
				$('#error-terminal').hide().html('');
					...
					$('#error-terminal').show().html('<div class="warning"><strong>Warning:</strong> Total already 100.');
				}
			}			
		}
	...
	$('.save-btn').livequery('click',function(){
		...
		if(source=='tbGenComp'){
			...
			if(parseInt(itm_bse)>100||parseInt(itm_bse)<1){
				var msg ="";
		...
		if(meas_obj!=null){
			$.each(meas_obj,function(i,result){
				...
				var col = '<tr col-ctr='+clctr+' class="measurable">';
					...
					col += '<td class="micro noborder"><a href="javascript:void();" class="delete-btn" inside="tbMeasItem"></a></td>';
					col +='</tr>';
