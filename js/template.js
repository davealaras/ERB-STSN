	...
	$('.add-btn').click(function(){
		...
		if(source=='tbGenComp'){
			...
			if(v =="###"){
				...
				$('#error-terminal').show().html('<div class="warning"><strong>Warning:</strong> Percentage is required.');
				$('#percentage').focus();
			}else if(total+parseInt($('#percentage').val())>100){
				$('#error-terminal').show().html('<div class="warning"><strong>Warning:</strong> Excess percentage');
				$('#percentage').focus();
			}else{
				if(duplicate==true){
				...
				$('#error-terminal').hide().html('');
					...
					$('#description').prepend('<option value="###" class="default" selected >Select Description</option>');
